nuevas_columnas_fede = function(dataset){
  ...
  dataset[,m_prestamos_totales := rowSums( cbind(mprestamos_hipotecarios,mprestamos_personales,mprestamos_prendarios) , na.rm=TRUE ) ]
  ...
  dataset[,m_invierte := rowSums( cbind(mplazo_fijo_dolares,mplazo_fijo_pesos,minversion1_dolares,minversion1_pesos,minversion2) , na.rm=TRUE ) ]
  dataset[,m_adeuda := rowSums( cbind(mv_msaldototal,mprestamos_personales,mprestamos_prendarios) , na.rm=TRUE ) ]
  dataset[,rt_invierte_adeuda := m_invierte/m_adeuda]
  dataset[,m_egresos := rowSums( cbind( mautoservicio, mpagodeservicios, mforex_buy, mtransferencias_emitidas, mextraccion_autoservicio, mtarjeta_visa_consumo, mtarjeta_master_consumo, na.rm = TRUE))]
  dataset[,m_egreso_principal := pmax( mautoservicio, mpagodeservicios, mforex_buy, mtransferencias_emitidas, mextraccion_autoservicio, mtarjeta_visa_consumo, mtarjeta_master_consumo, na.rm = TRUE)]
  dataset[,pc_egreso_principal := m_egreso_principal/m_egresos ]
